...
export default async function newDeck(args) {
  ...
  if (!name) {
    ...
    console.error('  --cinema: instala mira-cinema.js + GSAP (câmera, profundidade, grade, tecla A)');
  ...
  if ('cinema' in flags) {
    ...
    /* ORDEM DAS TAGS: o GSAP antes do mira-cinema.js, e os dois antes dos
       ...
       estiver carregado, e a ordem é o que garante isso. */
    let doc = readFileSync(join(dest, 'index.html'), 'utf8');
    if (!doc.includes('mira/mira-cinema.js')) {
      const tags = '<script defer src="assets/vendor/gsap.min.js"></script>\n'
  ...
  if (cinema) {
    console.log('  Cinema: mira-cinema.js + GSAP instalados. Câmera, profundidade, grade e tecla A disponíveis.');
  }
  if (offline) {
