import { join } from 'path';
import { existsSync, mkdirSync, cpSync, readFileSync, writeFileSync, readdirSync } from 'fs';
import { MIRA_ROOT, PROJECT_ROOT, loadConfig, saveConfig } from '../utils/paths.js';
import { applyOffline } from '../../templates/vendor/offline-core.mjs';
import { ensureResponsive } from '../utils/responsive.js';

// Lidos do disco, não fixos no código: um template gerado pelo /mira-image-template e copiado
// para templates/decks/ passa a valer na hora, sem editar esta lista. O base.css e o
// responsive.css não são temas, são a camada compartilhada.
const NAO_SAO_TEMAS = new Set(['base', 'responsive']);

// o template padrão do Mira: aparece SEMPRE em primeiro na lista, marcado
// como recomendado. Lista alfabética esconderia a recomendação no meio.
export const DECK_PADRAO = 'mira-default';

// Templates que trazem o próprio tema e NÃO aceitam --theme. O bloco
// @MIRA:THEME deles existe para o contrato de esqueleto do /mira-fast, não
// para ser substituído: injetar tema + base.css neles atropelaria o layout
// (o base.css impõe altura fixa e fundo ao palco e ao body).
export const THEME_AGNOSTIC = new Set([
  'sandeco-just-animation-template',
  'mira-studio-demo',
  'mira-studio-full-demo',
  'mesa-tatica',
]);

function listarDecks() {
  const dir = join(MIRA_ROOT, 'templates', 'decks');
  if (!existsSync(dir)) return [];
  const todos = readdirSync(dir, { withFileTypes: true })
    .filter(d => d.isDirectory() && existsSync(join(dir, d.name, 'index.html')))
    .map(d => d.name)
    .sort();
  return todos.includes(DECK_PADRAO)
    ? [DECK_PADRAO, ...todos.filter(n => n !== DECK_PADRAO)]
    : todos;
}

// rótulo para a lista impressa: o padrão vem anotado
function rotularDecks(decks) {
  return decks.map(n => (n === DECK_PADRAO ? n + ' (recomendado)' : n)).join(', ');
}

function listarThemes() {
  const dir = join(MIRA_ROOT, 'templates', 'themes');
  if (!existsSync(dir)) return [];
  return readdirSync(dir)
    .filter(f => f.endsWith('.css'))
    .map(f => f.slice(0, -4))
    .filter(n => !NAO_SAO_TEMAS.has(n))
    .sort();
}

const DECKS = listarDecks();
const THEMES = listarThemes();

export default async function newDeck(args) {
  const config = loadConfig();
  if (!config) {
    console.error('\n  Mira não está instalado nesta pasta. Execute "npx mira-animator install" primeiro.\n');
    process.exit(1);
  }

  const positional = args.filter(a => !a.startsWith('--'));
  const flags = Object.fromEntries(
    args.filter(a => a.startsWith('--')).map(a => {
      const [k, v] = a.slice(2).split('=');
      return [k, v ?? true];
    })
  );

  const name = positional[0];
  if (!name) {
    console.error('\n  Uso: npx mira-animator new <nome> [--deck=...] [--theme=...] [--cinema]\n');
    console.error('  --cinema: instala mira-cinema.js + mira-foco.js + GSAP + servidor.bat (câmera na tecla C, ritmo na tecla A)');
    console.error(`  Decks:  ${rotularDecks(DECKS)}`);
    console.error(`  Themes: ${THEMES.join(', ')}\n`);
    process.exit(1);
  }

  // mira-default: título em cima e animação ocupando o slide inteiro. É o
  // padrão do Mira; os outros templates continuam disponíveis via --deck.
  const deck = flags.deck ?? DECK_PADRAO;
  const theme = flags.theme ?? config.defaultTheme ?? 'mira-dark';

  if (!DECKS.includes(deck)) {
    console.error(`\n  Deck desconhecido: "${deck}". Opções: ${DECKS.join(', ')}\n`);
    process.exit(1);
  }
  if (!THEMES.includes(theme)) {
    console.error(`\n  Tema desconhecido: "${theme}". Opções: ${THEMES.join(', ')}\n`);
    process.exit(1);
  }

  // A pasta pode já existir como esqueleto: o /mira-new cria decks/<name>/references/
  // antes de perguntar template e tema, para o usuário ter onde soltar as referências
  // desde o primeiro minuto. O que caracteriza deck já montado é o index.html, não a pasta.
  const dest = join(PROJECT_ROOT, 'decks', name);
  if (existsSync(join(dest, 'index.html'))) {
    console.error(`\n  O deck "${name}" já existe em decks/${name}.\n`);
    process.exit(1);
  }

  mkdirSync(dest, { recursive: true });

  // 1. Copia o esqueleto do deck
  const deckSrc = join(MIRA_ROOT, 'templates', 'decks', deck, 'index.html');
  let html = readFileSync(deckSrc, 'utf8');

  // 2. Injeta o tema escolhido (inline, para o deck ser auto-contido).
  //    Exceção: os templates de THEME_AGNOSTIC mantêm o próprio bloco
  //    @MIRA:THEME e ignoram --theme. O animation-pure porque é multicor (a cor
  //    vive na paleta do <script>); os Studio e a mesa tática porque o layout
  //    deles não sobrevive ao base.css que viria junto do tema.
  //    O mira-default NÃO entra aqui: ele aceita tema de propósito (é o padrão
  //    do Mira, trocar de tema tem que funcionar). Ele neutraliza no próprio
  //    CSS o que o base.css impõe ao palco (altura fixa, glow, cantos).
  const themeAgnostic = THEME_AGNOSTIC.has(deck);
  if (!themeAgnostic) {
    const themeCss = readFileSync(join(MIRA_ROOT, 'templates', 'themes', `${theme}.css`), 'utf8');
    const baseCss = readFileSync(join(MIRA_ROOT, 'templates', 'themes', 'base.css'), 'utf8');
    const antes = html;
    html = html.replace(
      /\/\* @MIRA:THEME:START \*\/[\s\S]*?\/\* @MIRA:THEME:END \*\//,
      '/* @MIRA:THEME:START */\n' + themeCss + '\n\n' + baseCss + '\n/* @MIRA:THEME:END */'
    );
    // Sem o par de marcadores o replace não casa e devolve a string intacta.
    // Falhar em silêncio aqui já custou um bug: avise em vez de prometer.
    if (html === antes) {
      console.warn(`\n  Aviso: o template "${deck}" não tem o bloco @MIRA:THEME.`);
      console.warn(`  O tema "${theme}" NÃO foi aplicado. Acrescente os marcadores ao template,`);
      console.warn(`  ou declare-o em THEME_AGNOSTIC se ele traz o próprio tema de propósito.`);
    }
  }

  // 2.1. Camada de responsividade (reflow mobile-first). Vale para TODO deck,
  //      inclusive o animation-only: é só reflow, não impõe cor nem tema.
  //      Garante que o slide 16:9 fique legível no celular sem mobile.html separado.
  html = ensureResponsive(html).html;

  writeFileSync(join(dest, 'index.html'), html, 'utf8');

  // 2.5. Copia as ferramentas de autoria para <deck>/mira/ (diretiva: a raiz do
  //      deck fica só com index.html e launchers; JS de apoio vive em mira/).
  //      Os templates já referenciam <script src="mira/mira-*.js">.
  //      - mira-edit.js: modo edição (reordenar slides) — tecla E
  //      - mira-draw.js: telestrator (desenhar sobre o slide) — tecla P
  //      - mira-edit-free.js: edição livre estilo Canva (mover/redimensionar/
  //        rotacionar/duplicar/excluir/editar texto) — junto do modo edição (E)
  //      A references/ entra aqui junto: é pasta obrigatória de todo deck (diretiva de
  //      estrutura), então quem chama o CLI direto também nasce com ela.
  mkdirSync(join(dest, 'mira'), { recursive: true });
  mkdirSync(join(dest, 'references'), { recursive: true });
  for (const tool of ['mira-edit.js', 'mira-draw.js', 'mira-edit-free.js']) {
    const src = join(MIRA_ROOT, 'templates', 'authoring', tool);
    if (existsSync(src)) cpSync(src, join(dest, 'mira', tool));
  }

  // 2.6. Modo cinema (--cinema): câmera por viewBox, profundidade por planos,
  //      grade de cor e ritmo por slide. É OPT-IN por decisão de produto: o
  //      cinema só vira padrão depois de se provar, e um deck que não o pede
  //      não carrega o peso do GSAP.
  //      A cópia é feita aqui, no CLI, e não pelo agente: instalar biblioteca
  //      é passo determinístico, e pedir para um LLM copiar arquivo e injetar
  //      tag na ordem certa é onde isso falharia em silêncio.
  let cinema = null;
  if ('cinema' in flags) {
    const modulo = join(MIRA_ROOT, 'templates', 'authoring', 'mira-cinema.js');
    const gsap = join(MIRA_ROOT, 'templates', 'vendor', 'gsap.min.js');
    if (!existsSync(modulo) || !existsSync(gsap)) {
      console.error('\n  --cinema indisponível: falta mira-cinema.js ou gsap.min.js na instalação.');
      console.error('  Rode "npx mira-animator update".\n');
      process.exit(1);
    }
    mkdirSync(join(dest, 'assets', 'vendor'), { recursive: true });
    cpSync(modulo, join(dest, 'mira', 'mira-cinema.js'));
    cpSync(gsap, join(dest, 'assets', 'vendor', 'gsap.min.js'));

    /* O MODO CÂMERA (tecla C) vem junto, e não é opcional dentro do cinema:
       a câmera que o Motion Score escreve quase nunca fica certa de primeira,
       porque enquadramento é decisão de olho. Sem o mira-foco.js o autor não
       tem como ajustar cue nenhum, e a tecla C não faz nada. */
    const foco = join(MIRA_ROOT, 'templates', 'authoring', 'mira-foco.js');
    if (existsSync(foco)) cpSync(foco, join(dest, 'mira', 'mira-foco.js'));

    /* ORDEM DAS TAGS: o GSAP antes do mira-cinema.js, e os dois antes dos
       módulos de autoria. O módulo aborta com erro claro se o GSAP não
       estiver carregado, e a ordem é o que garante isso. O mira-foco.js vem
       depois do mira-cinema.js, e desiste sozinho se não achar a câmera. */
    let doc = readFileSync(join(dest, 'index.html'), 'utf8');
    if (!doc.includes('mira/mira-cinema.js')) {
      let tags = '<script defer src="assets/vendor/gsap.min.js"></script>\n'
        + '<script defer src="mira/mira-cinema.js"></script>\n';
      if (existsSync(foco)) tags += '<script defer src="mira/mira-foco.js"></script>\n';
      const alvo = doc.match(/<script defer src="mira\/mira-edit\.js"><\/script>/);
      doc = alvo
        ? doc.replace(alvo[0], tags + alvo[0])
        : doc.replace('</body>', tags + '</body>');
      writeFileSync(join(dest, 'index.html'), doc, 'utf8');
    }

    /* O SERVIDOR LOCAL, e ele é regra do modo cinema, não conveniência.
       O Ctrl+S do modo câmera reescreve o index.html: por http://localhost
       é direto, por file:// depende do seletor do Chrome, pede permissão a
       cada sessão e não existe em outro navegador. Apresentar continua sendo
       duplo clique no index.html; autorar é por aqui.

       O servidor embarcado é o mira-serve (POST /__mira_save). Um servidor
       estático qualquer (python -m http.server) mostra o deck mas NÃO grava,
       e o Ctrl+S falha com "Failed to fetch": foi defeito real em
       2026-08-07, o bat antigo subia exatamente esse. */
    const serve = join(MIRA_ROOT, 'lib', 'mira-serve.js');
    if (existsSync(serve)) cpSync(serve, join(dest, 'mira', 'mira-serve.mjs'));
    const bat = join(MIRA_ROOT, 'templates', 'authoring', 'servidor.bat');
    if (existsSync(bat)) {
      writeFileSync(
        join(dest, 'servidor.bat'),
        readFileSync(bat, 'utf8').replace(/__MIRA_DECK__/g, name),
        'utf8'
      );
    }
    cinema = true;
  }

  // 3. Modo offline por padrão: copia as libs vendoradas (já embarcadas na instalação)
  //    para decks/<name>/assets/vendor/ e aponta o <head> para elas. O deck nasce
  //    self-contained, abre por file:// e passa em firewall corporativo. Nada é
  //    baixado aqui — só copiado de templates/vendor/.
  const vendorSrc = join(MIRA_ROOT, 'templates', 'vendor');
  let offline = null;
  if (existsSync(vendorSrc)) {
    offline = applyOffline(dest, vendorSrc);
  }

  // 4. Registra no config
  // Substitui a entrada se o deck já constar (mesmo padrão do `link` com sources): recriar um
  // deck com o mesmo nome não pode deixar duas linhas contando histórias diferentes no config.
  config.decks = config.decks ?? [];
  const entrada = { name, template: deck, theme, createdAt: new Date().toISOString() };
  const idx = config.decks.findIndex(d => d.name === name);
  if (idx >= 0) config.decks[idx] = entrada;
  else config.decks.push(entrada);
  saveConfig(config);

  console.log(`\n  Deck "${name}" criado em decks/${name}/index.html`);
  console.log(`  Template: ${deck} | Tema: ${themeAgnostic ? 'próprio do template (theme-agnóstico)' : theme}`);
  if (cinema) {
    console.log('  Cinema: mira-cinema.js + mira-foco.js + GSAP instalados. Tecla C ajusta a câmera, tecla A o ritmo.');
    console.log('  Para AUTORAR, abra pelo servidor.bat (o Ctrl+S do modo câmera precisa de http://localhost).');
  }
  if (offline) {
    console.log(`  Offline: libs locais em assets/vendor/ (${offline.copied} arquivos) — abre por file:// sem internet.`);
  } else {
    console.log('  Aviso: bundle offline nao encontrado (templates/vendor/). Rode "npx mira-animator update".');
  }
  console.log('  Abra no navegador ou peça ao agente para preencher com uma fonte vinculada.\n');
}
