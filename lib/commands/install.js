...
export default async function install() {
  ...
  for (const engine of selectedEngines) {
    ...
    if (engine.id === 'claude-code') {
      if (answers.agents.includes('mira-fast')) writer.installClaudeWorkflow('mira-fast-engine');
      if (answers.agents.includes('mira-ultrafast')) writer.installClaudeWorkflow('mira-ultrafast-engine');
    }
  }
  ...
  spinner.succeed('  mira-templates/, decks/ e mira-plugins/ prontos');
  ...
  // `skillsDirs` existe para o sincronizador de plugins não precisar duplicar a
  // tabela de engines: ele roda no projeto do usuário, sem acesso ao pacote.
  const skillsDirs = [...new Set(selectedEngines.map(e => e.skillsDir))];
  ...
  // Ativa plugins que já estejam em mira-plugins/ (caso de reinstalação).
  const plugSync = syncPlugins(projectRoot);
  if (!plugSync.hasSessionHook && plugSync.enginesWithoutHook.length) {
    console.log(chalk.yellow('  !') + `  ${plugSync.enginesWithoutHook.join(', ')} não têm gancho de início de sessão.`);
    console.log(chalk.gray('     Rode "npx mira-animator plugin sync" ao criar ou apagar um plugin.'));
  }
  ...
  console.log(chalk.gray('    /mira-fast          deck inteiro numa chamada so'));
