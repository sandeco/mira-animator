...
export default async function editDeck(args) {
  ...
  // 1b. gravador nativo (tecla R): NÃO é injetado aqui — só decks de estúdio
  //     têm um, e qual dos dois (9:16 ou 16:9) depende do formato. Mas se o
  //     deck já tem o seu, ele é atualizado junto: correções do gravador (a
  //     grade CFR que conserta a dessincronia de áudio no Premiere, por
  //     exemplo) só chegam ao usuário quando a cópia dentro do deck troca.
  const gravadoresAtualizados = [];
  for (const rec of ['mira-record.js', 'mira-record-16x9.js']) {
    const destRec = join(deckDir, 'mira', rec);
    if (!existsSync(destRec)) continue;
    const srcRec = join(MIRA_ROOT, 'templates', 'authoring', rec);
    if (!existsSync(srcRec)) continue;
    cpSync(srcRec, destRec);
    gravadoresAtualizados.push(rec);
  }
  ...
  if (gravadoresAtualizados.length) {
    console.log(`  Gravador: ${gravadoresAtualizados.join(' + ')} atualizado (grade CFR: MP4 entra no Premiere sem drift de áudio)`);
  }
