...
export class Writer {
  ...
  // Copia o script de aviso de versão para `.mira/bin/` (Node puro, sem deps).
  installRuntime() {
    const dest = join(this.projectRoot, '.mira', 'bin', 'version-notice.js');
    this._mkdir(dirname(dest));
    cpSync(join(RUNTIME_DIR, 'version-notice.js'), dest, { force: true });
    this._register(dest);
  }
