...
/* memória do Mira: log append-only de evidência de edição, FORA do deck (o
   deck publicado é drop-and-run e levaria o perfil do usuário junto). */
const MEMORY_DIR = process.env.MIRA_MEMORY_DIR || join(homedir(), '.mira-memory');
...
function handlePrefs(req, res) {
    let raw = '';
    req.on('data', (c) => { raw += c; if (raw.length > 2 * 1024 * 1024) req.destroy(); });
    req.on('end', () => {
        try {
            const linhas = evidenciaLinhas(JSON.parse(raw));
            if (linhas.length) {
                mkdirSync(MEMORY_DIR, { recursive: true });
                appendFileSync(join(MEMORY_DIR, 'evidencia.jsonl'), linhas.join('\n') + '\n', 'utf8');
                console.log('  evidência:', linhas.length, 'linha(s)');
            }
            send(res, 200, JSON.stringify({ ok: true, linhas: linhas.length }), 'application/json');
        } catch (e) {
            // falhar a captura nunca pode atrapalhar o Salvar; o cliente ignora
            send(res, 500, 'erro ao gravar evidência: ' + e.message);
        }
    });
}
...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'POST' && req.url.split('?')[0] === '/__mira_prefs') return handlePrefs(req, res);
